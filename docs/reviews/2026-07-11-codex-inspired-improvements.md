...
**Status:** M1–M3 **done** (2026-07-12, `SCHEMA_VERSION` 4→5). M4–M6 remain
follow-ups.

| # | Pri | hop gap | Codex reference | Value |
|---|-----|---------|-----------------|-------|
| M1 | ✅ Done | No `model` captured; `turn_context` arm already parsed at `codex.rs:69-76` | `protocol.rs:3240` (`TurnContextItem.model`, real value e.g. `gpt-5.6-sol`); `SessionMeta.model_provider` `:3048` | A searchable/displayable **model** column. Cheap: the record is already parsed — add the field. Consider a matching Claude model source so the column is cross-agent (`B-010`). **Shipped:** `SessionSummary.model`, MODEL column, captured cross-agent (Codex `turn_context.model` first-wins; Claude assistant `message.model`, synthetic `<…>` sentinels skipped). |
| M2 | ✅ Done | `Git` struct omits commit hash | `GitInfo.commit_hash` `protocol.rs:3347` (real SHA on disk) | Enables showing/searching the commit a session ran against. One-field add. **Shipped:** `Git.commit_hash` → `SessionSummary.commit`, indexed `STRING\|STORED` (no default column; searchable/opt-in). Codex only. |
| M3 | ✅ Done | No `source`/`thread_source` filtering; every rollout indexed equally | `SessionMeta.source` (`SessionSource`, `protocol.rs:2720`), `thread_source` (`ThreadSource`, `:2737`); `INTERACTIVE_SESSION_SOURCES` allowlist `lib.rs:25-32` | Hide sub-agent / `memory_consolidation` / non-interactive threads a user would never resume. Mirror Codex's own interactive allowlist. Ties to `I-001` (empty-session skipping) as a sibling filter. **Shipped:** the engine skips non-interactive sessions via `Adapter::is_interactive` (Codex overrides it with a fail-open denylist; the marker vocabulary stays in the adapter, not the engine) beside the empty-session skip; `source` parsed loosely (bare string or `{"subagent":…}` object) so nested variants don't fail the line. |
