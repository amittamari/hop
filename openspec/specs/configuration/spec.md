...
Defines how `config.toml` settings shape TUI display behavior, in particular how
preview and session-list rendering are seeded from the `[display]` section and how
preview UI state is held for the session.
